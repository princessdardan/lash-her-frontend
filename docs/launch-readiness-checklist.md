...
- [ ] Embedded Afterpay merchant/location eligibility is confirmed in Square for the actual catalog. Verify the C$1–C$2,000 final-total range, unavailable/declined/cancelled flows and the separate service policy card using `docs/square-afterpay.md`. The invoice feature flag does not control embedded Afterpay.
- [ ] `TRAINING_AFTERPAY_SQUARE_INVOICE_ENABLED=true` is set only when the retained training Square Invoice endpoint is needed; otherwise it is unset or `false`.
...
- [ ] For live service booking, both `SERVICE_BOOKING_SQUARE_ENABLED=true` and `SERVICE_BOOKING_SQUARE_CARD_ON_FILE_ENABLED=true` are set. The card flow tokenizes with intent `CHARGE_AND_STORE`; full-payment Afterpay uses its own token and a separate `STORE` policy-card token. Both confirm through `/api/booking/payment/confirm`. Disabling direct card-on-file payment fails new payment sessions closed; it does not create a new hosted Payment Link.
