...
- [ ] No deployed configuration or readiness check depends on `CHITCHATS_BRANCH_ID`; the region-aware config (`CHITCHATS_REGION` plus the source-controlled intake location) is authoritative.
...
- [ ] The shipping-teardown migrations (`0062`–`0066`) are understood to be **irreversible** `DROP TABLE`/`DROP COLUMN` operations on tables/columns that hold production rows (attestation, duty-assignment, funding-review, service-policy, tax-policy, manual-policy, and intake-location records; the `product_shipment_jobs` funding columns; and `product_shipments.intake_location_attestation_id`). Capture a verified pre-drop snapshot/row-count of those tables before applying in production; the retained source config supersedes their runtime use, but confirm no audit/compliance retention obligation before dropping.
...
- [ ] **Training checkout:** Complete a paid training checkout through the Square card flow (Web Payments SDK). If `TRAINING_AFTERPAY_SQUARE_INVOICE_ENABLED=true`, also verify the Square invoice path creates and publishes a Square invoice, that `invoice.payment_made` routes to the training Square Invoice finalizer before service-booking/no-show fallback, and that the enrollment is finalized idempotently.
...
Shipping/checkout readiness is now **source-controlled config, not owner-attested DB records**. The former per-duty assignments, step-up attestations, funding reviews, and the versioned calendar / service-policy / tax-policy / manual-policy / provider-certification / intake-location-attestation DB rows were removed; the operational values they carried live in `src/lib/shipping/product-shipping-config.ts` and `src/lib/commerce/product-tax-policy.ts`, versioned in git. Legal/regulatory policy is observed operationally by the owner. Change-detection between quote and checkout-commit is version-based (`policyVersion` / `taxPolicyVersion`).
...
- [ ] The configured intake location reflects the actual physical location where parcels first enter the Chit Chats network; the owner has confirmed the region and location against the live account. There is no longer a DB attestation record, step-up, or `validUntil` to verify.
...
- [ ] Service-policy `insuranceLimitCents` and `signatureCapable` in `PRODUCT_SHIPPING_SERVICE_POLICIES` are verified against Chit Chats' current published per-service coverage (the code defaults are conservative CAD 100 placeholders marked `VERIFY`). Bump `PRODUCT_SHIPPING_POLICY_VERSION` on any change.
...
- [ ] The U.S. DDU contract (`PRODUCT_SHIPPING_US_DDU_CONTRACT`) and manual (studio pickup) cancellation policy (`PRODUCT_MANUAL_CANCELLATION_POLICY`) are populated, and their disclosure/policy **text** and schema **versions** are **confirmed with the business/legal owner** before production. U.S. product SKU `usRegulatoryCertification.*Version` values must match the contract's `version` / `tariffMetadataSchema.version` / `fdaRequirements.version` (enforced by `product-checkout-eligibility`; see `docs/us-shipping-regulatory-data-contract.md`). The contract's `effectiveFrom`/`effectiveUntil` window is **not enforced** (managed outside the storefront) — but each U.S. product's `usRegulatoryCertification.validUntil` **is** still enforced.
...
| Square commerce webhook        | Verify `/api/webhooks/square` receives and accepts a product/training `payment.*` event and reconciles the order (idempotently).                                                                                                                                                                                                            | Vercel log/event ID, accepted signature, idempotency key, and redacted payment reference.                                                                                                                                                                                                                                                                                                                                                                                                                                       |        |
