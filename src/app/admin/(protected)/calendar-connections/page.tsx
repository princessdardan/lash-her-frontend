...
export default async function AdminCalendarConnectionsPage({
  ...
  const discoveredEntries: Array<
    readonly [string, AdminGoogleCalendarOption[]]
  > = await Promise.all(
    data.connections.map(async (connection) => {
      ...
      try {
        return [
          connection.id,
          await listConnectedGoogleCalendars(connection.id),
        ] as const;
      } catch {
        return [connection.id, [] as AdminGoogleCalendarOption[]] as const;
      }
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-lh-muted">
            Connect a Google account, then assign one or more canonical
            calendars to each booking resource. One active calendar receives
            bookings; any number can contribute busy time.
          </p>
        </div>
        {canManage ? (
      ...
      <section className="grid gap-5 xl:grid-cols-2">
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-lh-muted">{connection.id}</p>
                </div>
                <StatusPill
                  tone={
              ...
              {canManage ? (
                <form
                  ...
                  className="mt-4 flex flex-wrap items-end gap-2 rounded-xl border border-lh-line p-3"
                >
                  <input
                    type="hidden"
                    name="connectionId"
                  ...
                  <Field label="Credential ownership">
                    <select
                      ...
                      name="employeeUserId"
                      defaultValue={connection.credentialOwnerAdminUserId ?? ""}
                    >
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  ...
                  {connection.status !== "disabled" ? (
                    <form action={disableCalendarConnectionAction}>
                      ...
                      <ConfirmSubmitButton
                        className={secondaryButtonClass}
                        confirmation="Disable this connection and all of its calendar assignments?"
                      >
                        Disable connection
                      </ConfirmSubmitButton>
                    </form>
              ...
              {canManage && connection.status === "active" ? (
                ...
                >
                          ...
                          {calendars.map((calendar) => (
                            <option key={calendar.id} value={calendar.id}>
                              ...
                              {canGoogleCalendarAcceptBookings(
                                ...
                                ? calendar.accessRole
                                : `${calendar.accessRole} (busy only)`}
                            </option>
                          ))}
                        </select>
                      </Field>
                    ) : (
                      <Field label="Canonical calendar ID">
                        <input
                          className={inputClass}
                          name="providerCalendarId"
                          required
                  ...
                  <div className="mt-4 flex flex-wrap gap-5 text-sm">
                    ...
                    <label className="flex items-center gap-2">
                      <input name="acceptsBookings" type="checkbox" /> Receives
                      new bookings
                    </label>
                  </div>
                  <button
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-lh-muted">
                        {assignment.contributesBusy ? "Busy" : ""}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {canManage && assignment.status === "active" ? (
                        <form action={disableCalendarAssignmentAction}>
                          ...
                          <ConfirmSubmitButton
                            className={secondaryButtonClass}
                            confirmation="Disable this calendar assignment?"
                          >
                            Disable
                          </ConfirmSubmitButton>
                        </form>
        ...
        {data.connections.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-lh-line bg-white p-6 text-lh-muted">
            No Google Calendar connections. Connect the owner&apos;s account to
            begin.
          </p>
        ) : null}
      </section>
