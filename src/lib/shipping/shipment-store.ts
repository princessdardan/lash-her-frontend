...
export async function recheckShipmentPurchaseFunding(input: {
  ...
  return getPrivateDb().transaction(async (tx) => {
    ...
    if (
      ...
      return false;
    if (executionContext.activeShipmentId !== job.shipmentId) return false;
    // Funding gate removed — no local balance reservation. The execution-context
    // checks above are the purchase preconditions.
    return true;
  });
}
