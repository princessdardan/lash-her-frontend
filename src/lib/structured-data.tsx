...
export function buildServiceJsonLd(service: TServiceEditorial): JsonLdObject {
  return {
    "@context": "https://schema.org",
    "@type": "Service",
...
export function buildTrainingProgramCollectionJsonLd(
  ...
  const itemListElement = programs.map((program, index) => ({
    ...
    item: {
      ...
      image: getImageUrl(
        program.seo?.image || program.image || program.heroImage,
      ),
      provider: businessReference(),
...
export function buildTrainingProgramJsonLd(
...
function getOpeningHours(
  ...
  const hours = contactData?.blocks
    ...
    ?.hours?.map((entry) =>
      [entry.days, entry.times].filter(Boolean).join(" ").trim(),
    )
    .filter(Boolean);
  ...
  if (variantPrices.length > 1) {
    ...
    if (lowPrice === highPrice) {
      return buildOffer({
        availability:
          product.isAvailable &&
          variantPrices.some((variant) => variant.isAvailable),
        price: lowPrice,
        priceCurrency: product.currency,
        url: `${SITE_URL}/products/${product.slug}`,
      });
...
function getServiceUrl(service: TServiceEditorial): string {
  return `${SITE_URL}/services/${service.slug}`;
}
