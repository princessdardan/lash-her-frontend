...
describe("ProductCard", () => {
  it("renders availability label, fulfillment note, variant option title, SKU, price, and unavailable label", async () => {
    process.env.NEXT_PUBLIC_SANITY_DATASET = "test-dataset";
    process.env.NEXT_PUBLIC_SANITY_PROJECT_ID = "test-project";

    const { ProductCard } = await import("./product-card");

    const product: TSellableProduct = {
      _id: "prod-1",
      title: "Test Product",
    ...
    assert.ok(html.includes("[VAR-1-SKU]"), "Missing variant 1 SKU");
    assert.ok(html.includes("[VAR-2-SKU]"), "Missing variant 2 SKU");
