...
const LEGACY_PRODUCT_CART_STORAGE_KEY = "lash-her:product-cart:v1";
const PRODUCT_CART_STORAGE_KEY = "lash-her:product-cart:v2";

async function readStoredCartItems(
  page: Page,
): Promise<Array<{ productId: string; quantity: number }>> {
  return page.evaluate(
    ({ legacyKey, v2Key }) => {
      const rawCart =
        window.localStorage.getItem(v2Key) ??
        window.localStorage.getItem(legacyKey);
      if (!rawCart) return [];
      const parsed: unknown = JSON.parse(rawCart);
      if (Array.isArray(parsed))
        return parsed as Array<{ productId: string; quantity: number }>;
      if (
        parsed &&
        ...
        return (
          parsed as { items: Array<{ productId: string; quantity: number }> }
        ).items;
      }
      return [];
    },
    {
      legacyKey: LEGACY_PRODUCT_CART_STORAGE_KEY,
      v2Key: PRODUCT_CART_STORAGE_KEY,
    },
  );
}
...
async function selectRequiredProductOptions(page: Page): Promise<void> {
  ...
  const optionGroups = main
    .locator('[role="group"]')
    .filter({ has: page.getByRole("button") });
  const groupCount = await optionGroups.count();

  for (let index = 0; index < groupCount; index += 1) {
    const firstAvailableOption = optionGroups
      .nth(index)
      .locator("button:not([disabled])")
      .first();
    if (
      (await firstAvailableOption.count()) > 0 &&
      (await firstAvailableOption.isEnabled())
    ) {
...
test.describe("Product Detail Page", () => {
  test("should display product details, gallery, purchase controls, and link back to catalog", async ({
    page,
  }) => {
    ...
    await expect(
      page
        .getByRole("link", { name: /back to catalog|back to products/i })
        .first(),
    ).toBeVisible();
    ...
    await expect(
      page.getByText(
        /detail pages are editorial only|purchases remain inside the catalog cart flow/i,
      ),
    ).toHaveCount(0);
    ...
    if ((await addToCart.count()) > 0) {
      ...
      await expect(
        main.getByRole("button", { name: /buy now/i }),
      ).toBeVisible();
    }
  });

  test("should group option sections and avoid raw payment forms", async ({
    page,
  }) => {
    if (!(await openFirstProductDetail(page))) {
      test.skip(true, "No product detail page was available to open");
    ...
    const optionHeading = page
      .getByRole("heading", {
        name: /available options|choose option|options/i,
      })
      .first();
    if ((await optionHeading.count()) > 0) {
      await expect(optionHeading).toBeVisible();
      const optionGroups = main
        .locator('[role="group"][aria-labelledby], fieldset, section')
        .filter({
          has: page.getByRole("heading", {
            name: /option|size|style|format|bundle|deposit|payment/i,
          }),
        });
      await expect(optionGroups.first()).toBeVisible();
      await expect(
        main.getByRole("button", { name: /add to cart/i }),
      ).toBeVisible();
      await expect(
        main.getByRole("button", { name: /buy now/i }),
      ).toBeVisible();
    ...
    await expect(
      main.getByText(
        /stripe|card number|credit card number|cvc|cvv|expiry|expiration date/i,
      ),
    ).toHaveCount(0);
    await expect(
      main.getByLabel(/card number|credit card|cvc|cvv|expiry|expiration/i),
    ).toHaveCount(0);
    await expect(
      main.locator(
  ...
  test("should require a variant selection before PDP add to cart and buy now", async ({
    page,
  }) => {
    const hrefs = await getProductDetailHrefs(page);
    ...
    for (const href of hrefs.slice(0, 8)) {
      ...
      if ((await addToCart.count()) === 0 || (await buyNow.count()) === 0) {
        test.skip(
          true,
          "No product detail purchase controls were available to validate",
        );
        return;
      }
      ...
      await expect(
        main.getByText(/choose an available product option/i),
      ).toBeVisible();
  ...
  test("should add a PDP selection to cart and persist it across reload", async ({
    page,
  }) => {
    if (!(await openFirstProductDetail(page))) {
      test.skip(true, "No product detail page was available to open");
    ...
    if ((await addToCart.count()) === 0) {
      test.skip(
        true,
        "No add to cart button was available on the product detail page",
      );
      return;
    }
  ...
  test("should send only the Buy Now selection in the checkout request", async ({
    ...
    await page.addInitScript((key) => {
      ...
      window.localStorage.setItem(
        ...
        JSON.stringify({
          version: 2,
          items: [{ productId: "cart-only-product", quantity: 1 }],
    ...
    expect(storedCart).toContainEqual({
      productId: "cart-only-product",
      quantity: 1,
    });
  });

  test("should show saved cart after closing Buy Now instead of sticky Buy Now payload", async ({
    page,
  }) => {
    const hrefs = await getProductDetailHrefs(page);
    if (hrefs.length === 0) {
    ...
    if ((await buyNow.count()) === 0) {
      test.skip(
        true,
        "No buy now button was available on the second product detail page",
      );
      return;
    }
    if (await buyNow.isDisabled()) await selectRequiredProductOptions(page);
